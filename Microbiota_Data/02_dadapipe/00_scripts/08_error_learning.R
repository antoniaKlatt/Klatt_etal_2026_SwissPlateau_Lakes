...
###############################
#setwd("..")
#filtFs <- readRDS("07_filter_and_trim/RDS/filtFs_bacteria_BS05.RDS")
#filtRs <- readRDS("07_filter_and_trim/RDS/filtRs_bacteria_BS05.RDS")
#taxa <- "bacteria"
#run <- "BS05"
###############################
...
if(length(filtFs)>0){filtFs <- filtFs[file.exists(filtFs)]}
if(length(filtRs)>0){filtRs <- filtRs[file.exists(filtRs)]}
...
#check if taxa-run combination exist there is at least 1 forward and 1 reverse file
#if not skip everything
taxa_run <- paste(taxa, run, sep="_")
if (taxa_run %in% taxa_runs & length(filtFs)>0 & length(filtRs>0)) {
